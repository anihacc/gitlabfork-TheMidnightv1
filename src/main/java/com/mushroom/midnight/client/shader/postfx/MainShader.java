...
public class MainShader extends PostProcessingEffect {
    ...
    private final FloatBuffer modelViewBuffNIO = FloatBuffer.wrap(modelViewBuff);
    ...
    private void bind(int slot, int texture) {
        activeTexture(GL13.GL_TEXTURE0 + slot);
        bindTexture(texture);
        activeTexture(GL13.GL_TEXTURE0);
    }

    public void updateMatrices(MatrixStack mvStack) {
        glGetFloatv(GL_PROJECTION_MATRIX, projectionBuff);
        mvStack.getLast().getMatrix().write(modelViewBuffNIO);
